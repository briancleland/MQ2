$(document).ready(function(){
	...
	*/
	clues[0] = {id: 0, name:"Wishing Chair", done:false, 
              description:"In bronze this leaf holds the key<br>A wishing chair just made for me<br>The first of Betty's middle name<br>Will help the maker with my game", 
              latlng: [-45, 10]};
	clues[1] = {id: 1, name:"Bucket of Treasure", done:false, 
              description:"The bucket is somewhere, but can you find it and find out what is inside?!", 
              latlng: [-56, -29]};
	clues[2] = {id: 2, name:"Sea of the Undead", done: false, 
              description: "The sea of the undead hasn't been visited in over 200 years, will you be the first to see what's inside?", 
              latlng: [-50, 61]};	
	clues[3] = {id: 3, name:"Sugar Island of Doom", done:false, description:"Find the treasure chest and hopefully it hasn't been looted before!", latlng: [-60, 31]};
	clues[4] = {id: 4, name:"Toblerone of Terror", done:false, description:"Find the treasure chest and hopefully it hasn't been looted before!", latlng: [-42, 50]};
	clues[5] = {id: 5, name:"Haunted Gallows", done:false, description:"Find the treasure chest and hopefully it hasn't been looted before!", latlng: [-72, 10]};
	clues[6] = {id: 6, name:"Treasure Chest", done:false, description:"Find the treasure chest and hopefully it hasn't been looted before!", latlng: [-67, -10]};
  ...
  var arrayOfLatLngs = [];
    ...
    arrayOfLatLngs.push(clues[i].latlng);
	}
  ...
  // map.fitBounds(bounds);
	...
	for(var i = 0; i < clues.length; i++){
    var clueMarker = L.AwesomeMarkers.icon({
      icon: 'question',
      prefix: 'fa',
      markerColor: 'red'
    });
		var marker = L.marker(clues[i].latlng,{title: clues[i].name, riseOnHover: true, icon:clueMarker })
                  // .bindPopup('<a href="#clue' + i + '" data-role="button" data-mini="true" data-rel="dialog">' + clues[i].name + '</a>', {autoPan:false})
                  .addTo(map);
		console.log(i);
  ...
  L.marker([-20,12], {icon: greenMarker}).addTo(map);
